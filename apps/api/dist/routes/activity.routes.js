...
const router = Router();
router.post('/', requireAuth, upload.array('attachments', 5), // allow up to 5 files
async (req, res) => {
    try {
        const { customerName, type, summary, notes, activityDate, } = req.body;
...
router.get('/:id', requireAuth, async (req, res) => {
    try {
        const record = await ActivityService.getById(req.params.id);
        if (!record) {
