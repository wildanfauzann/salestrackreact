...
router.patch('/:id/approve', requireAuth, requireRole(['manager']), async (req, res) => {
    try {
        const record = await LeaveService.updateStatus(req.params.id, 'approved', req.user.id);
        res.json(record);
    }
    catch (error) {
...
router.patch('/:id/reject', requireAuth, requireRole(['manager']), async (req, res) => {
    try {
        const record = await LeaveService.updateStatus(req.params.id, 'rejected', req.user.id);
        res.json(record);
    }
    catch (error) {
